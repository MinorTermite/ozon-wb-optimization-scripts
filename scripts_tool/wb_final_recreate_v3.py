...
def final_recreation_v3():
    ...
    print(f"Re-creating {len(target_data)} cards with _v3 suffix.")

    upload_url = 'https://content-api.wildberries.ru/content/v2/cards/upload'
    
    for i, card in enumerate(target_data):
        # We use _v3 to avoid collision with existing _v2 drafts
        new_vendor_code = f"{card.get('vendorCode', '')}_v3"
