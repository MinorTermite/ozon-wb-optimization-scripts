...
def targeted_lookup():
    ...
    # Pick top 5 nmIDs that were supposedly recovered or active
    nmids = [c.get('nmID') for c in cards[:5] if c.get('nmID')]
    print(f"Checking specific nmIDs: {nmids}")
    
    # Fetch details for these specific nmIDs
    ...
    if r.status_code == 200:
        ...
        print(f"Found {len(found)} out of {len(nmids)} targeted nmIDs.")
        for f_card in found:
            print(f"- Found {f_card.get('nmID')} ({f_card.get('vendorCode')})")
    else:
