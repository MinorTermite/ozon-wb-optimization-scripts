...
def final_recreation_v4():
    ...
    print(f"Re-creating {len(target_data)} cards with _v4 suffix (Fixed Weight).")

    upload_url = 'https://content-api.wildberries.ru/content/v2/cards/upload'
    
    for i, card in enumerate(target_data):
        # We use _v4 to ensure a fresh validation state
        new_vendor_code = f"{card.get('vendorCode', '')}_v4"
        ...
        # 2. Dimensions & Weight
        # Crucial: weightBrutto must be > 0. Bracelets are ~100-200g.
        dims = card.get('dimensions', {})
        weight = dims.get('weightBrutto') or 100 # Default 100g if missing
        ...
        new_dimensions = {
            ...
            "weight": int(weight) # WB V2 uses "weight" field inside dimensions for some subjects
        }
        # To be absolutely sure, we include both if the API allows or just the valid one
        # Based on previous inspection, it was "weightBrutto" in the GET response.
        # But in UPLOAD, it's often nested or a characteristic.
        # Let's add "Вес с упаковкой (кг)" characteristic too as it's common.
        has_weight_char = any("Вес" in c.get('name', '') for c in chars)
        if not has_weight_char:
            chars.append({
        ...
        # 4. Sizes (Generate new barcodes)
        new_sizes = []
        for s in card.get('sizes', []):
            new_sizes.append({
                "techSize": str(s.get('techSize', "0")),
                "wbSize": str(s.get('wbSize', "")),
                "skus": [] # This forces WB to generate unique barcodes
            })
        ...
        # Note: I'm putting Weight in characteristics "Вес с упаковкой (кг)" which is the most reliable way for Bijouterie.
